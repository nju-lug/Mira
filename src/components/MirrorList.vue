...
const extraColumns = computed(() => store.state.isMobile ? [] : [
  ...
  },
  {
    ...
    key: 'lastUpdate',
    align: 'center',
    render: data => data.lastUpdate ? timeFromNow(
      data.lastUpdate, locale.value as 'zh' | 'en'
    ) : '-'
  },
  {
    ...
    key: 'nextUpdate',
    align: 'center',
    render: data => data.nextUpdate ? timeFromNow(
      data.nextUpdate, locale.value as 'zh' | 'en'
    ) : '-'
  }
